...
Inside the `<app-root>` tag you see the text "Loading...". Everything inside the tag will be rendered by the browser after Angular compiles the application. When Angular is done, you will no longer see "Loading..." on the screen. Instead, you will see the `<app-root>` component and its content, which is:
